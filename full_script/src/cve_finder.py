            ...
            for config in configurations:
                for node in config.get("nodes", []):
                    for cpe in node.get("cpeMatch", []):
                        ...
                        cve = CVE(cve_id, exploitability_score, impact_score)
...
def get_product_version(entry, server_name):
    ...
    return versions.get(server_name, entry.get("version", ""))

folder = Path("./cache/cve_indexes")
...
def process_file(input, output):
    ...
    for entry in entries:
        product_str = entry.get("server", "").lower()
        product = get_product_mapping(product_str)
        version = get_product_version(entry, product_str)
